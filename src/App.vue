<template>
  <section
    class="h-[100vh] w-full space-y-10 bg-sky-100 flex flex-col justify-center items-center"
    :class="[isSubmit && 'bg-green-100']"
  >
    <h1 data-greet="greet" class="text-3xl">
      Good Morning{{ greetingMsg && ", " + greetingMsg }}
    </h1>
    <form
      data-form="form"
      @submit.prevent="submitHandler"
      class="shadow-lg bg-sky-50 p-5 rounded-lg w-[450px]"
    ...
    >
      <div class="space-y-3 pt-3">
        <div class="flex justify-between p-3">
          ...
          <input
            data-name="name"
            v-model="userName"
            class="border border-transparent focus:outline-none focus:border-sky-200 ring-1 focus:ring-sky-400 rounded-sm"
            :class="[
              isSubmit &&
      ...
      <div class="space-y-6 my-10">
        ...
        <ul data-HAY="HAY" class="flex justify-evenly">
          <li
            :class="[isSubmit && 'bg-green-100']"
            class="bg-sky-100 p-2 px-4 rounded-md space-x-2"
          >
            <input v-model="picked" id="Awsome" value="Awsome" type="radio" />
          ...
          <li
            ...
            class="bg-sky-100 p-2 px-4 rounded-md space-x-2"
          >
            <input v-model="picked" id="Great" value="Great" type="radio" />
    ...
    <p data-success="success" v-if="isSubmit" class="text-green-800">
      **{{ userEmail }} successfully subscribed the newsletter.
    </p>
    <p data-errorMsg="errorMsg" v-if="showErrorMsg" class="text-red-800">
